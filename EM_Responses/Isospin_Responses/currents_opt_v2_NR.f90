...
!
    ...
    if(i_fl.eq.1) then
       ...
       !NR below
       Pi_k1(:,:)=matmul(gamma_mu(:,:,5),k1_sl(:,:))/(-sum(k1(2:4)**2)-xmpi**2)
       Pi_k2(:,:)=matmul(gamma_mu(:,:,5),k2_sl(:,:))/(-sum(k2(2:4)**2)-xmpi**2)
...
!   &     *(pd2-xmd**2)/((pd2-xmd**2)**2+xmd**2*gd**2)
    ...
    !NR below
             RSa(:,:,i,j)=matmul(id4(:,:),g_munu(i,j)*id4(:,:)-matmul(gamma_mu(:,:,i),gamma_mu(:,:,j))/3.0d0) & !- &
    !&    2.0d0*pa(i)*pa(j)/3.0d0/xmd**2*id4(:,:)-(gamma_mu(:,:,i)*pa(j)-gamma_mu(:,:,j)*pa(i))/3.0d0/xmd) &
    &    *(1.0d0/(xmn - xmd_a))
!   &     *(pa2-xmd**2)/((pa2-xmd**2)**2+xmd**2*ga**2)

         RSb(:,:,i,j)=matmul(id4(:,:),g_munu(i,j)*id4(:,:)-matmul(gamma_mu(:,:,i),gamma_mu(:,:,j))/3.0d0) &!- &
    !&    2.0d0*pb(i)*pb(j)/3.0d0/xmd**2*id4(:,:)-(gamma_mu(:,:,i)*pb(j)-gamma_mu(:,:,j)*pb(i))/3.0d0/xmd) &
    &    *(1.0d0/(xmn - xmd_b))
!   &     *(pb2-xmd**2)/((pb2-xmd**2)**2+xmd**2*gb**2)
         RSc(:,:,i,j)=matmul(id4(:,:),g_munu(i,j)*id4(:,:)-matmul(gamma_mu(:,:,i),gamma_mu(:,:,j))/3.0d0) &!- &
    !&    2.0d0*pc(i)*pc(j)/3.0d0/xmd**2*id4(:,:)-(gamma_mu(:,:,i)*pc(j)-gamma_mu(:,:,j)*pc(i))/3.0d0/xmd) &
    &    *(1.0d0/(xmn - xmd_c))
!   &     *(pc2-xmd**2)/((pc2-xmd**2)**2+xmd**2*gc**2)
         RSd(:,:,i,j)=matmul(id4(:,:),g_munu(i,j)*id4(:,:)-matmul(gamma_mu(:,:,i),gamma_mu(:,:,j))/3.0d0) &!- &
    !&    2.0d0*pd(i)*pd(j)/3.0d0/xmd**2*id4(:,:)-(gamma_mu(:,:,i)*pd(j)-gamma_mu(:,:,j)*pd(i))/3.0d0/xmd) &
    &    *(1.0d0/(xmn - xmd_d))
!   &     *(pd2-xmd**2)/((pd2-xmd**2)**2+xmd**2*gd**2)
...
subroutine det_J1Jdel_exc(gd,res_re,nuc1_iso)
   implicit none
 ...
 subroutine det_J1Jdel_exc_nr(gd,res_re,nuc1_iso)
   implicit none
   integer*4 :: i1,f1,i2,f2,i,j,it1,it2
   integer*4 :: nuc1_iso !This is the isospin of the struck nucleon
   complex*16 :: nuc1_isospinor(2)
   ...
   pi_prop1 = (1.0d0/(sum(k1(2:4)**2) + xmpi**2))
   pi_prop2 = (1.0d0/(sum(k2(2:4)**2) + xmpi**2))

   k1crossq = cross_product(k1,q)
   k2crossq = cross_product(k2,q)

   do i=1,3
    do i1=1,2
      do f1=1,2
        do i2=1,2
            J1(i,f1,i1) = pi_prop1*sum(sig(1:3,f1,i1)*k1(2:4))*( k1crossq(i) - &
            &   ( sum(sig(1:3,i2,i2)*k1(2:4)) ) )
        enddo
      enddo
    enddo
   enddo
   ...
   return
 end subroutine det_J1Jdel_exc_nr
