program em_2body
   ...
   integer*4 :: nw,nev,i,xA,i_fg,np,ne,j,i_fsi,nwlk,nwfold
   ...
   if (myrank().eq.0) then
      ...
      fname='C12_QMC_noFSI_EM_1b_'//trim(en_char)//'_'//trim(theta_char)//'.out'
      fname=trim(fname)
   ...
   call mc_init(i_fg,i_fsi,irn,nev,nwlk,xpf,thetalept,xmpi,xmd,xmn,xA,np,ne,nk_fname)
   allocate(w(nw),sig(2,2,nw),sig_err(2,2,nw))
   ...
   do i=1,nw
      ...
      !! I want only the electromagnetic
        write(6,*) 'wval and ph sp', w(i), sig(1,1,i),sig(2,1,i)
        write(7,*)w(i), sig(1,1,i),sig(2,1,i)
        !flush(7)
      endif  
   enddo
   !close(7)
   ...
   if(i_fsi.eq.1) then
       ...
       allocate(sig_fold(2,2,nw),wfold(nwfold),fold(nwfold))
       do i=1,nwfold
       ...
       enddo
       do i=1,nw
          write(6,*) w(i),sig_fold(1,1,i),sig_fold(2,1,i)
          !write(7,*) w(i),sig_fold(1,1,i),sig_fold(2,1,i)
       enddo
    endif
    close(7)
