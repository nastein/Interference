...
subroutine mc_init(i_fg_in,i_fsi_in,irn_in,nev_in,nwlk_in,xpf_in,thetalept_in,xmpi_in,xmd_in,xmn_in,xA_in, &
...
subroutine mc_eval(ee,w,sig_avg_tot,sig_err_tot)
  ...
  use mympi
   ...
   real*8 :: w,emax,ee
   real*8 :: sig_o(2,2,nwlk)
   real*8 :: sig_avg(2,2),sig_err(2,2)
   real*8 :: sig_avg_tot(2,2),sig_err_tot(2,2)
...
subroutine f_eval(ee,p1,ip1,ie1,w,sig)
  ...
  sig(1,:)=sig0*(v_ll*r_now(:,1))*1.e9
  sig(2,:)=sig0*(v_t*r_now(:,4))*1.e9
  return
...
!   ffgnd=gep/sqrt(1.0d0-q2/(xmn+xmd)**2)/sqrt(1.0d0-q2/l3)
   ...
   call nform(-q2/hbarc**2,f_1p,f_1n,f_2p,f_2n,gep,gen,gmp,gmn)
...
!   call det_J1Jdel_exc(had_del_p)
!   call det_J1Jpi_exc(had_pi_p)
   
   call det_Ja(f_1n,f_2n)
   call hadr_tens(res_n)

!   call det_J1Jdel_exc(had_del_n)
!   call det_J1Jpi_exc(had_pi_n)
   ...
   exc(1)=had_del_p(1,1)+had_pi_p(1,1)-had_del_n(1,1)-had_pi_n(1,1)
   exc(2)=0.0d0
   exc(3)=0.0d0
   exc(4)=had_del_p(2,2)+had_del_p(3,3)-had_del_n(2,2)-had_del_n(3,3)+had_pi_p(2,2)+had_pi_p(3,3)-had_pi_n(2,2)-had_pi_n(3,3) 
   !&     + had_del_del(2,2) + had_del_del(3,3)        
   exc(5)=0.0d0
