module mc_module
   implicit none
   integer*4, private, save :: nev,xA,i_fg,i_fsi,np,ne,nwlk,npot,np_del,i_intf,nenu,nt
   integer*4, private, parameter :: neq=10000,nvoid=100
   real*8, private, save ::  xpf,xpmax
   real*8, private, save:: xmpi,xmd,xmn,norm,cos_theta_min,cos_theta_max
   character*40, private,save:: flux_fname
   real*8, private, save:: enu_max, flux_norm
   ...
   real*8, private, allocatable :: pv(:),dp(:,:),ep(:),Pke(:,:),nk(:),enu_v(:),flux_v(:)
...
subroutine mc_init(i_intf_in,i_fg_in,i_fsi_in,irn_in,nev_in,nwlk_in,xpf_in, &
   & cos_theta_min_in,cos_theta_max_in,nt_in,xmpi_in,xmd_in,xmn_in,xA_in, &
   &  np_in,ne_in,flux_fname_in,nenu_in)

  use mathtool
   implicit none
   integer*8 :: irn_in(nwlk_in)
   integer*4 :: nev_in,nwlk_in,xA_in,i_fg_in,np_in,i,j,ne_in,np0,ne0,ien,i_intf_in,nenu_in,nt_in
   integer*4 :: ipot,i_fsi_in
   real*8 :: xpf_in,xmpi_in,xmd_in,xmn_in,mlept_in,hp,he,cos_theta_min_in, cos_theta_max_in,dummy,enu_1,enu_2,henu
   ...
   xmd=xmd_in
   xmn=xmn_in
   cos_theta_min=cos_theta_min_in
   cos_theta_max=cos_theta_max_in
   nt=nt_in
   xA=xA_in
   i_fg=i_fg_in
   np0=np_in
...
subroutine mc_eval(tmu,sig_avg_tot,sig_err_tot)
  ...
  use mympi
   ...
   real*8 :: w,emax,enu,emu,tmu,flux,enu_i,costhetalept
   ...
   do i=1,nev
      do j=1,nwlk
         ...
         if (i.ge.neq.and.mod(i,nvoid).eq.0) then
            ...
            enu = tmu + xmmu + (enu_max - tmu - xmmu)*ran()
            !enu = 1000.0d0
            w = enu - tmu - xmmu
            !w=tmu
            if(w.lt.0.0d0)cycle
            ...
            sig_o(:,j) = 0.0d0
            do ic=1,nt 
               costhetalept = cos_theta_min+dble(ic-0.001)*(cos_theta_max-cos_theta_min)/dble(nt)
               !costhetalept = 0.342d0!cos(pi*30.0d0/180.0d0)
               !write(6,*)'costhetalept = ', costhetalept
               !write(6,*)'costhetalept = ', costhetalept
               call f_eval(enu,costhetalept,pv(ip1_o(j)),ip1_o(j),ie1_o(j),w,sig_o_ct(:,j)) 
               !write(6,*)'sig(',costhetalept,') = ', sig_o_ct(:,j)
               sig_o(:,j) = sig_o(:,j) + sig_o_ct(:,j)
               !write(6,*)'sig_o = ', sig_o(:,j)
            !enddo
  
            sig_o(:,j)=sig_o(:,j)*flux*enu_i/g_o(j)/flux_norm/6.0d0/nt
            !sig_o(:,j)=sig_o(:,j)/g_o(j)
            !write(6,*)'sig_o = ',sig_o(:,j), flux,enu_i,flux_norm,nt
...
subroutine f_eval(ee,costhetalept,p1,ip1,ie1,w,sig)
   use mathtool
  implicit none
  integer*4 :: ip1,ie1
  real*8 :: ee,ctpp1,p2,ctp2,phip2,p1,ctp1,phip1,w,q2
  real*8 :: jac_c,qval,kdotq,elept,xklept,costhetalept
  real*8 :: v_ll,v_cc,v_cl,v_t,v_tp
  real*8 :: tnl2,sig0,delta,rho,rhop
  real*8 :: qp(4), sig(2), r_now(2,5)

  !write(6,*)'w = ', w, ', ee = ', ee, ', costhetalept = ', costhetalept
  ...
  q2=2.0d0*ee*elept - 2.0d0*ee*xklept*costhetalept - xmmu**2
  !write(6,*)'q2 = ', q2
  if(q2.lt.0.0d0) stop
  qval=sqrt(q2+w**2)
...
subroutine int_eval(p2,ctp2,phip2,p1,phip1,ip1,ie1,w,qval,r_now)
   ...
   real*8 :: dir(5)
   real*8 :: had_del(4,4),had_pi(4,4),had_del_del(4,4)
   real*8 :: exc(5),r_now(2,5)
   complex*16 :: res(4,4)
...
!.......currents
   ...
   exc(5)=(-ci/2.0d0)*(had_del(2,3) - had_del(3,2) + had_pi(2,3) - had_pi(3,2))
